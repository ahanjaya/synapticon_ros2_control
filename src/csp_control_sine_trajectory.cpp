...
void simpletest(const char *ifname) {
    ...
    const double traj_duration = 7.5;                                              // seconds for a full cycle
    const int num_points = static_cast<int>(traj_duration / cycle_time) + 1;
    static std::vector<std::vector<double>> trajectory;
    ...
    if (ec_init(ifname)) {
        ...
        if (ec_config_init(false) > 0) {
            ...
            for (size_t joint_idx = 1; joint_idx < (num_joints + 1); ++joint_idx) {
                // Verify slave name
                if (&(ec_slave[joint_idx].name[0]) != &EXPECTED_SLAVE_NAME[0]) {
                        ...
                        switch (state) {
                            ...
                            case DELAY:
                                if (++delay_counter >= delay_cycles) {
                                    state = GENERATE_TRAJECTORY;  // Move to next state
                                    delay_counter = 0;
                                }
                            ...
                            case GENERATE_TRAJECTORY: {
                                ...
                                trajectory.resize(num_points, std::vector<double>(num_joints, 0.0));
                                traj_idx = 0;

                                for (int i = 0; i < num_points; ++i) {
                                    t = i * cycle_time;
                                    for (int j = 0; j < num_joints; ++j) {
                                        // Synchronous sine wave for all joints
                                        trajectory[i][j] = 0.5 * (M_PI) * std::sin(2 * M_PI * t / traj_duration);
                                    }
                                }
                                std::cout << "Trajectory initialized with " << num_points << " points." << std::endl;
                            ...
                            case NORMAL_OPERATION: {
                                // Directly set TargetPosition for each joint from the trajectory (no S-curve)
                                for (int s = 1; s <= num_joints; ++s) {
                                    double target_rad = trajectory[traj_idx][s - 1];
                                    int32_t target_enc = rad_to_encoder(target_rad, mechanical_reductions[s - 1], encoder_resolutions[s - 1]);
                                    out_somanet[s]->TargetPosition = target_enc;
                                }
                                traj_idx += 1;

                                if (traj_idx >= num_points) {
                                    state = DELAY;
                                }
                                break;
                            }
