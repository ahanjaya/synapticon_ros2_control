...
void simpletest(const char *ifname) {
  ...
  if (ec_init(ifname)) {
    ...
    if (ec_config_init(false) > 0) {
      ...
      ec_SDOwrite(slave, 0x6081, 0, FALSE, sizeof(profile_velocity), &profile_velocity, EC_TIMEOUTRXM);
      // Set Profile Acceleration (0x6083)
      ec_SDOwrite(slave, 0x6083, 0, FALSE, sizeof(profile_acceleration), &profile_acceleration, EC_TIMEOUTRXM);
      // Set Profile Deceleration (0x6084)
      ec_SDOwrite(slave, 0x6084, 0, FALSE, sizeof(profile_deceleration), &profile_deceleration, EC_TIMEOUTRXM);
      // Set Quick Stop Deceleration (0x6085)
      ec_SDOwrite(slave, 0x6085, 0, FALSE, sizeof(quick_stop_decel), &quick_stop_decel, EC_TIMEOUTRXM);
        ...
        for (i = 1; i <= 10000; i++) {
          ...
          if (wkc >= expectedWKC) {
            ...
            // Sending profile Position command
              ...
              // Always send the command, update target every 200 cycles
              if (i % 1000 == 0) {
                last_target = in_somanet_1->PositionValue + 100000 * (toggle ? 1 : -1);
                toggle = !toggle;

                // Set "new set-point" bit (bit 4) motor will move on rising edge
                out_somanet_1->TargetPosition = last_target;
                out_somanet_1->Controlword = 0b00011111;
                printf("\nNew target position: %d\n", last_target);
              } else {
                // Keep sending the last target, clear "new set-point" bit
                out_somanet_1->TargetPosition = last_target;
                out_somanet_1->Controlword = 0b00001111;
              }
            }
